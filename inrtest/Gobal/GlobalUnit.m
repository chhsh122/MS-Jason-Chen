...
static GlobalUnit *_manager = nil;

@implementation GlobalUnit

+ (GlobalUnit *)shareManager {

//    @synchronized (self) {
        if (!_manager) {
            _manager = [[GlobalUnit alloc] init];
        }
//    }
    return _manager;
}
...
+(void)showPopHint:(NSString *)contentMessage targetView:(UIView*)popView showInView:(UIView*)showView
{
    ...
    /* Some options to try.
     */
    popTipView.backgroundColor = Guoguo_backgroundColor;
    popTipView.borderColor = Guoguo_bordColor;
    popTipView.textColor = Guoguo_darkGreyFontColor;
...
+(id)addTitleContentArrowViews:(UIView*)parentView font:(UIFont*)font title:(NSString*)title contentText:(NSString*)content contentX:(NSInteger)contentX contentClass:(NSString*)contentClass txtDelegate:(id<UITextFieldDelegate>)txtDelegate showArrow:(BOOL)showArrowImg
{
    ...
    lblTitle.text = title;
    lblTitle.font = font;
    lblTitle.textColor = Guoguo_darkGreyFontColor;
    ...
    if (contentX>=0) {
        ...
        if ([GlobalUnit isBlankString:content]) {
        ...
        if ([[contentClass lowercaseString] isEqualToString:@"uilabel"]) {
            ...
            ((UILabel*)viewContent).textColor = Guoguo_darkGreyFontColor;
            ...
            ((UITextField*)viewContent).textColor = Guoguo_darkGreyFontColor;
